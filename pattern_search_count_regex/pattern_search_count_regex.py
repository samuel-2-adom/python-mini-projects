...
def find_first(pattern,file_name = "pg345.txt"):
...
def count_first(pattern,file_name = "pg345.txt"):
    count = 0
    with open(file_name,encoding = 'utf-8') as file:
        for line in file:
            result = re.search(pattern,line)
        ...
        try:
            ...
            if user_input == "00":
                ...
                if os.path.exists(file_name):
                    ...
                    result = f_first(pattern_input,file_name)
                    if result:
                        print()
                    ...
                    #count pattern
                    pattern = f'{pattern_input}'
                    count = c_first(pattern,file_name)
                    if result:
                        console.print(f"[underline bold yellow]Count[/underline bold yellow][bold magenta] - {count}")
                    else:
                        console.print(f"❗[bold red]exception,Count of [/bold red][bold magenta]'{pattern}' = 0")
